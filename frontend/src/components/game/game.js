...
class Game extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      playerNames: [
        { name: "Player1", color: "darkred" },
        { name: "Player2", color: "green" },
        { name: "Player3", color: "blue" },
        { name: "Player4", color: "#faf60a" }
      ]
    };
  ...
  componentDidMount() {
    ...
    try {
      ...
      toast("GAME " + localStorage.currentGameId + " LOADED", {
  ...
  movePawn(player, dice1, dice2) {
    ...
    blocks[players[player].position].playersOnBlock = _.filter(
      blocks[players[player].position].playersOnBlock,
      obj => {
    ...
    this.setState({
      players,
      blocks,
      currentBlock: blocks[newPlayerPosition],
  ...
  applyLogic(player, newPlayerPosition) {
    ...
    if (blockInfo.type === "PROPERTY") {
      ...
      toast(players[player].name + " paid 500 for bail", {
        type: toast.TYPE.INFO
      });
      if (players[player].balanceMoney <= 0) {
        this.eliminatePlayer(player, players);
      ...
      toast(players[player].name + " paid 493 as Income Tax", {
        type: toast.TYPE.INFO
      });
      if (players[player].balanceMoney <= 0) {
        this.eliminatePlayer(player, players);
      ...
      toast(players[player].name + " paid 2662 as Super Tax", {
        type: toast.TYPE.INFO
      });
      if (players[player].balanceMoney <= 0) {
        this.eliminatePlayer(player, players);
  ...
  processChance() {
    ...
    if (this.state.currentChance.move) {
      ...
      const currentPosition = this.state.players[this.state.currentPlayer]
        .position;
      let moves = 0;
    ...
    if (this.state.currentCommunity.move) {
      ...
      if (this.state.currentCommunity.buttonTag === "PAY") {
        ...
        toast(
          players[player].name +
            " paid " +
            this.state.currentCommunity.pay +
            " for COMMUNITY CHEST",
          { type: toast.TYPE.INFO }
        );
        if (players[player].balanceMoney <= 0) {
        ...
        toast(
          players[player].name +
            " collected " +
            this.state.currentCommunity.get +
  ...
  mortgage = propertyId => {
    ...
    players[playerindex].properties = _.filter(player.properties, property => {
      console.log(property.id + "b " + propertyId);
  ...
  closeNewGameModal(e) {
    ...
    switch (e.target.name) {
      case "new":
        ...
        break;
      case "resume":
        this.setState({ newGameModelOpen: false, gameIdModalOpen: true });
        break;
    }
  }
  playerOnChange = e => {
  ...
  startGame = async e => {
    e.preventDefault();
    const response = await axios.post(config.host + "/startNewGame", {
      state: emptyState,
      winner: null
    });
  ...
  resumeGame = async e => {
    ...
    if (this.state.gameId === undefined) {
      ...
      const response = await axios.post(config.host + "/getGame", {
      ...
      if (response.data.code == 1) {
        ...
        toast(response.data.data.gameId + " loaded", {
      ...
      <div className="App">
        ...
        >
          ...
          <button
            name="resume"
            class="newGameBtn"
            onClick={this.closeNewGameModal}
          >
            Resume Game
          </button>
          <button
        ...
        >
          ...
          >
            <input
              className="player-red-field"
              onChange={this.playerOnChange}
              value={this.state.playerNames[0].name}
              type="text"
              name="0"
            ...
            <br></br>
            <input
              className="player-green-field"
              onChange={this.playerOnChange}
              value={this.state.playerNames[1].name}
              type="text"
              name="1"
            ...
            <br></br>
            <input
              className="player-blue-field"
              onChange={this.playerOnChange}
              value={this.state.playerNames[2].name}
              type="text"
              name="2"
            ...
            <br></br>
            <input
              className="player-yellow-field"
              onChange={this.playerOnChange}
              value={this.state.playerNames[3].name}
              type="text"
              name="3"
        ...
        >
          ...
          <form
            method="post"
            className="players-form"
            onSubmit={this.resumeGame}
          >
            <input
              className="gameId-field"
        ...
        <Modal show={this.state.propBuyModalOpen} handleClose={this.closeModal}>
          <Card
          ...
          <div className="buy-modal-right">
            ...
            <div className="pass" onClick={this.closeModal}>
              PASS
            </div>
          </div>
        </Modal>
        ...
        >
          ...
          <button onClick={this.processCommunityChest}>
            {this.state.currentCommunity.buttonTag}
          </button>
        </Modal>
        ...
        <div className="right">
          <div className="appbar">
            <button onClick={this.saveState} className="appbtn">
              Save Game
            </button>
            <button onClick={this.exitGame} className="appbtn">
              Exit Game
            </button>
          </div>
          ...
          <div className="content">
            <div className="player-row1">
              <PlayerCard
                ...
                currentPlayer={
                  this.state.players[this.state.currentPlayer].id === 0
                }
              ...
              <PlayerCard
                ...
                currentPlayer={
                  this.state.players[this.state.currentPlayer].id === 1
                }
            ...
            <div className="player-row2">
              <PlayerCard
                ...
                currentPlayer={
                  this.state.players[this.state.currentPlayer].id === 2
                }
              ...
              <PlayerCard
                ...
                currentPlayer={
                  this.state.players[this.state.currentPlayer].id === 3
                }
